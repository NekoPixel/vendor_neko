...
func init() {
	android.RegisterModuleType("neko_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = nekoExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = nekoExpandVariables(ctx, dep_file)
	...
	cmd := nekoExpandVariables(ctx, String(g.properties.Cmd))
